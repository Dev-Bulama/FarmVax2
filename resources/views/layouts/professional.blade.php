        ...
        <aside id="sidebar" class="fixed lg:static inset-y-0 left-0 transform -translate-x-full lg:translate-x-0 w-64 bg-brand-dark shadow-lg min-h-screen sidebar-transition z-40 mt-16 lg:mt-0">
            <nav class="mt-5 px-3">
                ...
                <a href="{{ route('professional.telemedicine.index') }}"
                   class="group flex items-center px-3 py-3 text-sm font-medium rounded-lg mb-1 transition {{ request()->routeIs('professional.telemedicine.*') ? 'bg-brand-green text-brand-dark shadow-lg' : 'text-gray-200 hover:bg-brand-green hover:bg-opacity-20 hover:text-white' }}">
                    ...
                    @php
                        $_incomingCalls = \App\Models\TelemedicineRequest::where('professional_id', auth()->id())
                            ->whereIn('status', ['assigned', 'in_progress'])->count();
                    @endphp
                    @if($_incomingCalls > 0)
                        <span class="ml-auto bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full animate-pulse">{{ $_incomingCalls }}</span>
                    @endif
                </a>
