            ...
            <div>
                ...
                <div class="mt-4">
                    ...
                    <textarea name="address" id="address" rows="3"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2FCB6E] focus:border-transparent"
                        placeholder="Enter street address">{{ old('address', $user->address) }}</textarea>
                </div>
            </div>
...
if (countrySelect && stateSelect && lgaSelect) {
    ...
    const initialState = stateSelect.dataset.selected || '{{ $user->state_id ?? "" }}';
    const initialLga = lgaSelect.dataset.selected || '{{ $user->lga_id ?? "" }}';
            ...
            fetch(`/api/states/${countryId}`)
                ...
                .then(data => {
                    data.forEach(state => {
                        ...
                        option.value = state.id;
                        option.textContent = state.name;
                        if (state.id == initialState) option.selected = true;
                        stateSelect.appendChild(option);
                    });
                    stateSelect.disabled = false;
                    
                    // Trigger state change if there's an initial value
                    if (initialState) {
            ...
            fetch(`/api/lgas/${stateId}`)
                ...
                .then(data => {
                    data.forEach(lga => {
                        ...
                        option.value = lga.id;
                        option.textContent = lga.name;
                        if (lga.id == initialLga) option.selected = true;
                        lgaSelect.appendChild(option);
                    });
                    lgaSelect.disabled = false;
                });
