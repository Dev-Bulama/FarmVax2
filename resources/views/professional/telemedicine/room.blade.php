        ...
        <form action="{{ route('professional.telemedicine.complete', $req->id) }}" method="POST" class="space-y-2">
            ...
            <div class="flex gap-3">
                <button type="submit" onclick="endJitsiCall();return true;"
                        class="flex-1 py-3 bg-green-600 text-white font-bold rounded-xl text-sm">✅ Complete</button>
                <button type="button" onclick="endJitsiCall()"
                        class="w-14 h-12 rounded-xl bg-red-600 flex items-center justify-center">
                    <svg class="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M6.6 10.8c1.4 2.8 3.8 5.1 6.6 6.6l2.2-2.2c.3-.3.7-.4 1-.2 1.1.4 2.3.6 3.6.6.6 0 1 .4 1 1V20c0 .6-.4 1-1 1-9.4 0-17-7.6-17-17 0-.6.4-1 1-1h3.5c.6 0 1 .4 1 1 0 1.3.2 2.5.6 3.6.1.3 0 .7-.2 1L6.6 10.8z"/></svg>
                </button>
            </div>
        </form>
    </div>
...
<video id="remote-video" autoplay playsinline class="absolute inset-0 w-full h-full object-cover" style="display:none;z-index:1;"></video>
...
<div id="waiting-overlay" class="absolute inset-0 flex flex-col items-center justify-center"
    ...
    <div class="w-28 h-28 rounded-full border-4 border-green-500 flex items-center justify-center text-5xl mb-5 shadow-2xl" style="background:rgba(47,203,110,0.15);">🧑‍🌾</div>
    <p class="text-white text-2xl font-bold">{{ $farmerName }}</p>
    <p class="text-green-400 text-sm mt-1 animate-pulse">Waiting for patient…</p>
    @if($req->livestock)<p class="text-gray-500 text-xs mt-1">{{ $req->livestock->tag_number ?? ucfirst($req->livestock->livestock_type) }}</p>@endif
...
<div id="local-pip" class="absolute rounded-2xl overflow-hidden border-2 border-white/25 shadow-2xl"
     style="width:100px;height:148px;bottom:240px;right:14px;display:none;z-index:5;touch-action:none;cursor:move;">
...
<div class="absolute top-0 left-0 right-0 safe-top flex items-center justify-between px-4 py-3"
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <p class="text-white font-semibold text-sm">{{ $farmerName }}</p>
            <p id="call-timer" class="text-green-400 text-xs tabular-nums">Waiting…</p>
        </div>
    </div>
    <div class="flex items-center gap-2">
        <span id="conn-badge" class="px-2.5 py-1 rounded-full text-xs font-bold bg-yellow-500 text-black">⏳ Waiting</span>
...
<div class="absolute bottom-0 left-0 right-0 safe-bottom" style="z-index:10;background:linear-gradient(0deg,rgba(0,0,0,0.9) 0%,transparent 100%);">
    <div class="flex items-end justify-center gap-8 pt-8 pb-4 px-8">
    ...
    <form action="{{ route('professional.telemedicine.complete', $req->id) }}" method="POST" class="px-4 pb-2">
        @csrf
        <div class="bg-gray-900/90 rounded-2xl p-3 border border-gray-700/60">
            <p class="text-xs text-gray-400 mb-1 font-semibold uppercase tracking-wide">Notes</p>
            <textarea name="professional_notes" rows="2"
                      class="w-full bg-transparent text-sm text-white placeholder-gray-500 border-0 focus:ring-0 resize-none outline-none"
                      placeholder="Type diagnosis/treatment notes…"></textarea>
            <div class="flex justify-end mt-1">
                <button type="submit" onclick="return confirmComplete()"
                        class="px-5 py-1.5 bg-green-600 hover:bg-green-700 text-white font-bold rounded-xl text-sm transition">
                    ✅ Complete
                </button>
            </div>
        </div>
...
function endJitsiCall(){clearInterval(tid);}
</script>
@else
<script>
...
async function start() {
    ...
    pc.onconnectionstatechange = () => {
        ...
        else if(s==='disconnected') { setConn('🔴 Lost…','red'); }
        else if(s==='failed')       { setConn('🔴 Failed','red'); }
    };

    setConn('⏳ Waiting for farmer…','yellow');
    ...
    try {
        ...
        for(const sig of (data.signals||[])) {
            ...
            if(sig.type==='offer' && !hasOffer) {
                hasOffer=true;
                setConn('🟡 Answering…','yellow');
                await pc.setRemoteDescription(JSON.parse(sig.payload));
                const answer = await pc.createAnswer();
                await pc.setLocalDescription(answer);
                await postSig('answer', JSON.stringify(answer));
                ...
                setConn('🔴 Patient ended call','red');
                showHangupAlert();
            }
        }
    } catch(_) {}
...
function showHangupAlert() {
    ...
    div.innerHTML=`<div class="text-5xl mb-4">📵</div><p class="text-white text-xl font-bold mb-2">Call Ended</p><p class="text-gray-400 text-sm mb-6">The farmer has ended the session.</p><p class="text-gray-500 text-sm mb-4">Please add your consultation notes below.</p>`;
    document.getElementById('call-room').appendChild(div);
}

function startTimer() {
    const el=document.getElementById('call-timer');
    timerInterval=setInterval(()=>{timerSec++;el.textContent=fmt(Math.floor(timerSec/60))+':'+fmt(timerSec%60);},1000);
}
...
function showQuality() {
    ...
    badge.textContent=s.width>=1280?'HD':(s.width>=640?'SD':'Low');
...
function toggleMic() {
    ...
    document.getElementById('mic-icon').style.background=micOn?'rgba(255,255,255,0.18)':'rgba(239,68,68,0.8)';
...
function toggleCam() {
    ...
    document.getElementById('cam-icon').style.background=camOn?'rgba(255,255,255,0.18)':'rgba(239,68,68,0.8)';
...
(function(){
    ...
    pip.addEventListener('touchstart',e=>{const t=e.touches[0];ox=t.clientX;oy=t.clientY;sx=pip.offsetLeft;sy=parseInt(pip.style.bottom)||240;},{passive:true});
