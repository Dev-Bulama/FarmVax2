        ...
        <header class="bg-white shadow">
            <div class="px-4 sm:px-6 lg:px-8 py-4">
                <div class="flex justify-between items-center">
                    <div>
                        <h1 class="text-2xl font-bold text-blue-600">Service Requests</h1>
                        <p class="text-sm text-gray-600">Manage farmer service requests</p>
                    </div>
                </div>
            </div>
...
<p class="text-2xl font-bold text-gray-900">{{ $stats['total'] ?? 0 }}</p>
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <p class="text-2xl font-bold text-blue-600">{{ $serviceRequests->where('status', 'in_progress')->count() }}</p>
                </div>
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">{{ $request->user->name ?? 'Unknown' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-800">
                                        {{ ucfirst($request->service_type ?? 'Service') }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-sm">{{ Str::limit($request->description ?? 'No description', 40) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @php
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <a href="{{ route('professional.service-requests.show', $request->id) }}" class="text-blue-600 hover:text-blue-900">View</a>
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center">
                                    ...
                                    <p class="mt-2 text-sm text-gray-500">No service requests available</p>
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
